...
def coffee_machine():
    choice_of_service = input("Do you want a report/refill/order? ")
    if choice_of_service == "report":
        ...
        coffee_order = input("What would you like to order(espresso/latte/cappuccino)? ")
        if menu[coffee_order]["ingredients"]["water"] < resources["water"] and \
            ...
            if(input("Would you like to refill(Y/N)? ")) == "Y":
                refill()
            coffee_machine()
